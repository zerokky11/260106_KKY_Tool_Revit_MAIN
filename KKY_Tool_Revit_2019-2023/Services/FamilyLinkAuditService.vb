    ...
    Public NotInheritable Class FamilyLinkAuditService
        ...
        Public Shared Function Run(app As UIApplication,
            ...
            For i As Integer = 0 To total - 1
                Dim path As String = cleanedPaths(i)
                Dim doc As Document = Nothing

                Try
                    ReportProgress(progress, total, i + 1, 0.02R, $"문서 여는 중... {i + 1}/{total} {SafeFileName(path)}")

                    If String.IsNullOrWhiteSpace(path) Then Throw New ArgumentException("RVT 경로가 비어 있습니다.")
                    If Not File.Exists(path) Then Throw New FileNotFoundException("RVT 파일을 찾을 수 없습니다.", path)

                    doc = OpenProjectDocument(app.Application, path)
                    ...
                    Dim rvtName As String = SafeFileName(path)
                    ...
                    For fi As Integer = 0 To famTotal - 1
                        ...
                        AuditFamilyAsHost(doc, fam, path, targetMap, rows)
                    Next
