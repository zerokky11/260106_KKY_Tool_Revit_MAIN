...
Namespace KKY_Tool_Revit
    ...
    Friend Class AuditForm
        ...
        Private Sub BuildUi()
            ...
            depthPanel.Controls.Add(New Label() With {.Text = "MaxDepth:", .AutoSize = True, .Padding = New Padding(0, 8, 0, 0)})
        ...
        Private Sub OnScan(sender As Object, e As EventArgs)
            ...
            Dim cntOk = _rows.Count(Function(r) r.Issue = AuditIssueType.OK.ToString())
            Dim cntMiss = _rows.Count(Function(r) r.Issue = AuditIssueType.MissingAssociation.ToString())
            Dim cntSus = _rows.Count(Function(r) r.Issue = AuditIssueType.SuspiciousAssociation.ToString())
            Dim cntConf = _rows.Count(Function(r) r.Issue = AuditIssueType.SharedTypeDrivenConflict.ToString())
            Dim cntErr = _rows.Count(Function(r) r.Issue = AuditIssueType.Error.ToString())
            ...
            Try
                ...
                For Each fi As FamilyInstance In nestedInstances
                    ...
                    For Each t As Tuple(Of Parameter, ParamScope) In paramTuples
                        ...
                        Else
                            ' Heuristics: instance/type mismatch
                            ...
                            ' ParameterType mismatch (best-effort)
                            ...
                            ' Shared nested + type-driven control conflict (strictly for FamilyType linkage)
                    ...
                    ' Recursive scan: treat nested family as host too (separate host check)
        ...
        Private Shared Function OpenProjectDocument(app As Autodesk.Revit.ApplicationServices.Application, path As String) As Document
            ...
            ' Try: workshared-friendly open (best effort)
            Try
        ...
        Private Shared Function CollectParams(fi As FamilyInstance, includeTypeSelection As Boolean) As List(Of Tuple(Of Parameter, ParamScope))
            ...
            ' Type selection parameter (symbol id / type control)
            If includeTypeSelection Then
                Try
                    Dim pType As Parameter = fi.get_Parameter(BuiltInParameter.ELEM_TYPE_PARAM)
        ...
        Private Shared Function ShouldInspect(p As Parameter, scope As ParamScope, onlyImportant As Boolean) As Boolean
            ...
            ' Always inspect type-selection if included
            ...
            ' Important by name (KOR/ENG heuristic)
        ...
        Private Shared Function IsFamilyShared(fam As Family) As Boolean
            ...
            Try
                Dim p As Parameter = fam.get_Parameter(BuiltInParameter.FAMILY_SHARED)
